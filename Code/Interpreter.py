...
def Interpreter(Input_Folder, params_list):
	"""
	...
	for i, file in enumerate(filelist):
		if i ==0:
			...
			Arrays = np.expand_dims(Arrays,axis=Arrays.ndim)
		else:
			Arrays = np.concatenate((Arrays,np.loadtxt(file,delimiter=',').reshape(*Initial_Shape,1)),axis=2)
